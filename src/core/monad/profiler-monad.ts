...
export class ProfilerMonad<T> {
  constructor(
    private readonly computation: () => Promise<Result<T>>
  ) {}
   ...
   */
  map<U>(fn: (value: T) => U): ProfilerMonad<U> {
    return new ProfilerMonad(async () => {
      ...
      try {
        ...
        return failure(new ComputationError(
   ...
   */
  flatMap<U>(fn: (value: T) => ProfilerMonad<U>): ProfilerMonad<U> {
    return new ProfilerMonad(async () => {
      const result = await this.computation();
      ...
      try {
        ...
        return failure(new FlatMapError(
          error instanceof Error ? error : new Error(String(error))
        ));
      }
    });
   ...
   * @returns New monad representing the chained computation
   */
  chain<U>(fn: (value: T) => ProfilerMonad<U>): ProfilerMonad<U> {
   ...
   */
  tap(fn: (value: T) => void): ProfilerMonad<T> {
   ...
   */
  recover(fn: (error: Error) => T): ProfilerMonad<T> {
    return new ProfilerMonad(async () => {
      const result = await this.computation();
      ...
      try {
        ...
        return failure(new RecoveryError(
          result.error,
 ...
 */
export function profilerMonad<T>(
  computation: () => Promise<Result<T>>
): ProfilerMonad<T> {
...
export function pure<T>(value: T): ProfilerMonad<T> {
  return new ProfilerMonad(async () => success(value));
}
...
export function sequence<T>(
  monads: ProfilerMonad<T>[]
): ProfilerMonad<T[]> {
  return new ProfilerMonad(async () => {
    const results = await Promise.all(monads.map(m => m.run()));
    
    // Check if any failed
    const failures = results.filter(r => r.isFailure());
    if (failures.length > 0) {
      return failures[0] as any; // Return first failure
    }
    
    // All succeeded, extract values
    const values = results.map(r => (r as any).value as T);
